...
def validate_action(action, allow_game_progress=False):
    ...
    allowed_risks = {"navigation"} | ({"game_progress"} if allow_game_progress else set())
    ...
    if action["kind"] in {"tap", "long_press"} and re.search(r"agree to all|accept all|accetta tutto|consenti tutto", action.get("target", ""), re.I):
...
class Android:
    ...
    def launch(self):
        ...
        self.adb("shell", "am", "start", "-n", component[-1])
    ...
    def act(self, action, evidence, guard_path, *, allow_game_progress=False):
        validate_action(action, allow_game_progress)
