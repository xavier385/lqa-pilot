...
def fallback_template(path):
    # Runtime fallback for operators without the optional artifact runtime; same original client columns.
    from openpyxl import Workbook
    from openpyxl.styles import Font,PatternFill,Alignment
    from .report import CLIENT_HEADERS
    book=Workbook();sheet=book.active;sheet.title='Bug list';sheet.append(CLIENT_HEADERS)
    ...
    fields=['date','location','case_id','screenshot','type','description','id','leave_blank','leave_blank','owner','priority','category','leave_blank','leave_blank','leave_blank','status','leave_blank','leave_blank']
    for i in range(1,len(fields)+1):
        c=sheet.cell(2,i,'');c.alignment=Alignment(wrap_text=True,vertical='top')
        sheet.column_dimensions[c.column_letter].width=48 if i in (4,6) else 22
    sheet.row_dimensions[1].height=45;sheet.row_dimensions[2].height=300;sheet.freeze_panes='A2';book.save(path)
...
def import_project(path,folder,options,*,model=None,progress=None,cancelled=None):
    ...
    def reserve(role):
        ...
        if calls>=60: raise PilotError('Budget di lettura Excel esaurito: il gioco non è stato avviato. Suddividere il progetto.')
...
''',LAYOUT,{'sheets':summaries,'notes':notes,'language':options['language']})
    if layout['blockers'] and options.get('mode')!='video':raise PilotError('Formato cliente da chiarire: '+'; '.join(layout['blockers']))
    reports=layout['reports'] if options.get('mode')!='video' else [];validate_reports(reports,snapshot)
    if any('report' in s['roles'] for s in summaries) and not reports and options.get('mode')!='video':
        raise PilotError('È presente un report cliente ma non è stato mappato: il gioco non viene avviato')
    template=folder/'client-template.xlsx';fallback=False
    if not reports and options.get('mode')!='video':
        reports=[fallback_template(template)];fallback=True
    else: template.write_bytes(Path(path).read_bytes())
    entries={}
    for entry in taxonomy:
        if entry['id'] in entries and entries[entry['id']]!=entry:raise PilotError('Tassonomia cliente contraddittoria')
        entries[entry['id']]=entry
    tax={'source':Path(path).name,'entries':list(entries.values())} if entries else read_json(Path(__file__).parent/'data/client_taxonomy.json')
    if len(reports)==1:
    ...
    for start in range(0,len(cases),25):
        ...
        review=ask('''Review the plan against all extracted project context BEFORE Android starts. Return each input case ID exactly once.
...
ready=false only for a material contradiction, missing essential guide/resource, unresolved hierarchy or unusable objective.
...
''',REVIEW,{'cases':batch,'provenance':{c['id']:provenance[c['id']] for c in batch},'instructions':instructions,
            'glossary':glossary,'dev_keys':dev_keys,'log_keys':log_keys,'notes':notes,'report_layout':layout,'language':options['language']})
        if not review['ready'] or review['issues']:raise PilotError('Contesto da chiarire prima del gioco: '+'; '.join(review['issues'] or ['Analisi incompleta']))
    ...
    project={'name':Path(path).stem,'language':options['language'],'report_language':'English','mode':options.get('mode','full'),
        'device':{**options.get('device',{}),'package':options['package'],'serial':options.get('device',{}).get('serial','auto')},
        'cases':cases,'taxonomy':'taxonomy.json','glossary':glossary,'cheats':[dict(c,enabled=True) for c in cheats],
        'policy':{'allow_game_progress':bool(options.get('allow_game_progress',False))},
        ...
        'budgets':{'max_calls':options.get('max_calls',100),'max_actions':150,'max_minutes':90,'max_steps_per_case':45},
        ...
        'import_summary':{'ready':True,'sheets_read':len(summaries),'cases':len(cases),'glossary_terms':len(glossary),
            ...
            'fallback_report':fallback,'roles':role_counts}}
